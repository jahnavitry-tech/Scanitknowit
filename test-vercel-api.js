...
async function testAPI() {
  try {
    ...
    const ingredientsResponse = await fetch('http://localhost:3001/api/ingredients', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ text: 'Chocolate bar with almonds' })
    });
    ...
    const nutritionResponse = await fetch('http://localhost:3001/api/nutrition?query=chocolate');
    ...
    const redditResponse = await fetch('http://localhost:3001/api/reddit?query=chocolate');
