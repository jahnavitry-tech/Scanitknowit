  ...
  async fetch(req, env) {
    ...
    if (url.pathname === "/api/identify" && req.method === "POST") {
      try {
        ...
        const summaryResponse = await fetch("https://api.openrouter.ai/api/v1/chat/completions", {
          method: "POST",
          headers: {
            "Authorization": `Bearer ${env.OPENROUTER_API_KEY}`,
            "Content-Type": "application/json"
        ...
        return withCors(Response.json({ error: "Failed to identify product" }, { status: 500 }));
      }
    }
    if (url.pathname === "/api/ingredients" && req.method === "POST") {
      try {
        ...
        return withCors(Response.json({ error: "Failed to process ingredients" }, { status: 500 }));
      }
    }
    if (url.pathname === "/api/calories" && req.method === "GET") {
      try {
        ...
        return withCors(Response.json({ error: "Failed to fetch nutrition data" }, { status: 500 }));
      }
    }
    if (url.pathname === "/api/reddit" && req.method === "GET") {
      try {
        ...
        return withCors(Response.json({ error: "Failed to fetch Reddit data" }, { status: 500 }));
      }
    }
...
Question: ${question}` }
        ...
        return withCors(Response.json({ error: "Failed to process Q&A" }, { status: 500 }));
      }
    }
...
// .wrangler/tmp/bundle-Pkge7w/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-Pkge7w/middleware-loader.entry.ts
